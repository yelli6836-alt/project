...
router.get("/summary", asyncWrap(async (req, res) => {
  ...
  const itemIds = raw.split(",").map((x) => Number(String(x).trim())).filter((n) => Number.isFinite(n) && n > 0);
  ...
  const [rows] = await pool.query(
    `SELECT item_id, COUNT(*) AS review_count, AVG(star_score) AS avg_rating
       FROM item_review
...
router.get("/:itemId/summary", asyncWrap(async (req, res) => {
  ...
  res.json({ ok: true, item_id: itemId, summary: rows[0] || { review_count: 0, avg_rating: null } });
}));

router.get("/", asyncWrap(async (req, res) => {
  const itemId = Number(req.query.item_id);
  if (!Number.isFinite(itemId) || itemId <= 0) return res.status(400).json({ ok: false, error: "MISSING_OR_INVALID_ITEM_ID" });
  ...
  res.json({ ok: true, item_id: itemId, page, size, total: countRow.total, reviews: rows });
}));
