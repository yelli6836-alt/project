...
// helpers: treat NaN/invalid as defaults (demo-stable)
function toPosInt(v, def) {
  const n = Number.parseInt(String(v), 10);
  return Number.isFinite(n) && n > 0 ? n : def;
}
function toOptPosInt(v) {
...
router.get("/", asyncWrap(async (req, res) => {
  const categoryId = toOptPosInt(req.query.category_id);
  const q = req.query.q ? String(req.query.q).trim() : null;
  const status = req.query.status ? String(req.query.status).trim() : null;

  const page = toPosInt(req.query.page, 1);
  const size = Math.min(50, Math.max(1, toPosInt(req.query.size, 20)));
  const offset = (page - 1) * size;

  const where = [];
  const params = { limit: size, offset };

  if (categoryId !== null) { where.push("i.category_id = :categoryId"); params.categoryId = categoryId; }
...
router.get("/:itemId", asyncWrap(async (req, res) => {
  const itemId = toPosInt(req.params.itemId, -1);
  if (itemId <= 0) return res.status(400).json({ ok: false, error: "INVALID_ITEM_ID" });
